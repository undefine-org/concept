defmodule Concept.Objects.GuardContractTest do
  ...
  test "every registered guard implements icon/0 and render_config_form/2" do
    for mod <- Guards.all() do
      ...
      assert function_exported?(mod, :render_config_form, 2),
             "#{inspect(mod)} render_config_form/2"
    end
  end
  ...
  test "requires_proof config form renders the field key input" do
    html =
      render_component(
        fn assigns ->
          Concept.Objects.Guards.RequiresProof.render_config_form(
            %{"field" => "pr_url"},
            assigns.form
          )
        end,
  ...
  test "requires_approval config form renders approver options" do
    html =
      render_component(
        fn assigns ->
          Concept.Objects.Guards.RequiresApproval.render_config_form(
            %{"by" => "creator"},
            assigns.form
          )
        end,
