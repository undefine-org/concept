defmodule Concept.Pages.Notifiers.KnowledgeReindexTest do
  ...
  describe "KnowledgeReindex notifier" do
    test "creating a block enqueues IngestPage job with page_id", %{
      user: user,
      workspace: workspace
    } do
      {:ok, page} =
    ...
    test "creating blocks on different pages enqueues separate jobs", %{
      user: user,
      workspace: workspace
    } do
      {:ok, page_a} =
        Pages.create_page("Page A", workspace.id, nil, actor: user, tenant: workspace.id)

      {:ok, page_b} =
        Pages.create_page("Page B", workspace.id, nil, actor: user, tenant: workspace.id)
    ...
    test "archiving a page enqueues job with op: :delete", %{user: user, workspace: workspace} do
      {:ok, page} =
        Pages.create_page("To Archive", workspace.id, nil, actor: user, tenant: workspace.id)
    ...
    test "updating page metadata (rename) enqueues job with op: :upsert", %{
      ...
      {:ok, page} =
        Pages.create_page("Original Title", workspace.id, nil, actor: user, tenant: workspace.id)
