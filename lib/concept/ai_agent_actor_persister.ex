defmodule Concept.AiAgentActorPersister do
  use AshOban.ActorPersister

  def store(%Concept.Accounts.User{id: id}), do: %{"type" => "user", "id" => id}

  def lookup(%{"type" => "user", "id" => id}) do
    with {:ok, user} <- Ash.get(Concept.Accounts.User, id, authorize?: false) do
